...
- Светлая и тёмная темы получили более мягкую палитру, отдельные акцентные оттенки и заметные состояния выбранных элементов.
- Переключение разделов и смена темы теперь сопровождаются коротким плавным fade-переходом без анимации таблиц и графиков.
- Отчёты используют цвета из общей темы, а не старые жёстко заданные оттенки графика.
- Header отчётов адаптируется к узкой ширине, чтобы крупные числа и сравнение не сжимались.
- Progress bars в топе приложений используют compact-стиль без конфликта с общей высотой контролов.
- Статистика обновляется из live-снимка сборщика без лишней записи в SQLite при каждом обновлении интерфейса.
- Для больших локальных баз добавлен индекс под запросы пересечения интервалов и уменьшено чтение `boot_log`.

## Что нового в 1.3.2

- Экспорт CSV теперь отменяется, если текущие данные не удалось сохранить перед выгрузкой.
- CSV-файл заменяется только после успешной записи, поэтому сбой не оставляет повреждённый отчёт.
- Закрытие окна и сворачивание в трей стали устойчивее к временно заблокированной SQLite-базе.
- Правила категорий теперь одинаково распознают дубли с разным регистром и слешами в пути.
- Долгие операции приватности и очистки показывают busy-состояние и блокируют повторные клики.
- Интерфейс получил обновлённую нейтральную палитру, более заметные статусы, KPI и состояния кнопок.
...
2. В блоке **Assets** скачайте `PCStat.exe`. Это portable-версия: отдельная установка не нужна.
3. Запустите `PCStat.exe`. Python, Git и исходный код обычному пользователю не требуются.
4. При первом запуске Windows может показать предупреждение неизвестного издателя. Если файл скачан из официального release, подтвердите запуск вручную.

Проверить официальный CI-артефакт можно через GitHub/Sigstore provenance attestation:
...
- **Статистика** показывает активное время, текущую сессию, топ приложений и таблицу приложений за выбранный период.
- **Отчёты** помогают смотреть день, неделю, месяц, год или весь доступный период с переходом назад по календарю.
- **Категории** позволяют вручную отнести приложение к группе по имени exe или фрагменту пути.
- **Настройки** управляют темой, автозапуском, закрытием в трей, AFK-порогом, исключениями и сроком хранения истории.
- **Экспорт** создаёт CSV за выбранный период только после нажатия кнопки `Экспорт`.

Закрытие окна по умолчанию сворачивает приложение в трей, а сбор активности продолжается. Полный выход доступен через меню значка PC Stat в области уведомлений. Автозапуск включается вручную в настройках и создаёт запись только для текущего пользователя Windows.
